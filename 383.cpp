...
long long custom_pow(int a, int b) {
    if (b == 0) {
        ...
        if (b % 2 == 0) {
            int tmp = custom_pow(a, b / 2) % 1337;
            return ((tmp % 1337) * (tmp % 1337)) % 1337;
        } else {
            int tmp = custom_pow(a, b / 2) % 1337;
            return (tmp % 1337) * (tmp % 1337) * (a % 1337) % 1337;
        }
    }
}
...
int superPow(int a, vector<int>& b) {
    ...
    for (int i = 0; i < b.size(); i++) {
        res = (custom_pow(res, 10) * (custom_pow(a, b[i]))) % 1337;
    }
    return res % 1337;
}
